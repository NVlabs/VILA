...
def predict_captions(tokenizer, model, image_processor, context_len, model_name, video_file):
...
if __name__ == "__main__":
    ...
    tokenizer, model, image_processor, context_len, model_name = load_vila_model(args)

    for root, _, files in os.walk(args.parent_folder):
        for file in files:
            if file.endswith(".mp4"):  # Check if the file is an mp4 video
                current_folder = root
                file_name = file
                video_path = f"{current_folder}/{file_name}"

                predict_captions(tokenizer, model, image_processor, context_len, model_name, video_path)
